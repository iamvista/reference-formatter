# 學術文獻格式整理工具 - 完整開發覆盤與深度教學

> 從零到一：一個全端 Web 應用的完整開發歷程
>
> 作者：Vista
> 日期：2026-01-01
> 專案：Academic Reference Formatter

---

## 📚 文件導航

- [第一章：專案起源與需求分析](#第一章專案起源與需求分析)
- [第二章：技術架構深度解析](#第二章技術架構深度解析)
- [第三章：核心模組開發實戰](#第三章核心模組開發實戰)
- [第四章：Flask 應用開發](#第四章flask-應用開發)
- [第五章：前端交互設計](#第五章前端交互設計)
- [第六章：部署與 DevOps](#第六章部署與-devops)
- [第七章：問題偵錯案例集](#第七章問題偵錯案例集)
- [第八章：性能優化與最佳實踐](#第八章性能優化與最佳實踐)
- [第九章：延伸學習路線圖](#第九章延伸學習路線圖)

---

## 第一章：專案起源與需求分析

### 1.1 問題背景

學術寫作中，參考文獻的格式整理是一項**極度繁瑣但必不可少**的工作：

**現實困境：**
- 📝 不同期刊要求不同引用格式（APA、MLA、Chicago、Harvard...）
- ⏱️ 手動轉換耗時且容易出錯
- 🔄 投稿被拒後需要重新格式化所有文獻
- 📚 管理大量文獻（碩博士論文可能有 100+ 條）

**市場現況：**
- Zotero、Mendeley：功能強大但學習曲線陡峭
- 線上工具：多數收費或功能受限
- Word 內建工具：格式選擇有限

**我們的機會：**
打造一個**輕量級、免費、易用**的在線工具，專注解決格式轉換問題。

---

### 1.2 需求分析

#### 功能需求

**核心功能（MVP）：**
1. ✅ 解析文獻文本，提取結構化資訊
2. ✅ 支援 4 種主流格式（APA、MLA、Chicago、Harvard）
3. ✅ 一鍵切換格式顯示
4. ✅ 匯出為 TXT、DOCX、BibTeX、HTML

**增強功能：**
1. ✅ 透過 DOI 自動補完文獻資料
2. ✅ 批次處理多條文獻
3. ✅ 資料完整度和信心度評估

**未來擴展：**
- [ ] PDF 上傳並提取參考文獻
- [ ] 用戶系統（保存歷史記錄）
- [ ] 瀏覽器擴展（快速引用網頁）

#### 非功能需求

| 需求類型 | 目標 | 實現方式 |
|---------|------|---------|
| **性能** | 單次解析 < 2 秒 | 非同步 API 查詢 |
| **可用性** | 99% uptime | Render 免費方案 + 健康檢查 |
| **安全性** | HTTPS、無 XSS | Flask 內建防護 + HTTPS |
| **可維護性** | 模組化、低耦合 | 分層架構設計 |

---

### 1.3 技術選型決策

#### 後端：Flask vs Django vs FastAPI

| 框架 | 優點 | 缺點 | 選擇 |
|-----|------|------|-----|
| **Flask** | 輕量、靈活、學習簡單 | 功能需自行整合 | ✅ 選擇 |
| Django | 功能完整、自帶 ORM | 過於笨重、學習曲線陡 | ❌ |
| FastAPI | 現代、高性能、自動文檔 | 相對較新、生態較小 | ❌ |

**決策理由：** 專案規模小，不需要 Django 的全家桶；Flask 足夠輕量且生態成熟。

#### 前端：React vs Vue vs Vanilla JS

| 方案 | 優點 | 缺點 | 選擇 |
|-----|------|------|-----|
| **Vanilla JS** | 零構建、快速開發 | 複雜狀態管理困難 | ✅ 選擇 |
| React | 生態強大、組件化 | 需要構建工具、學習成本高 | ❌ |
| Vue | 漸進式、易學 | 仍需構建工具 | ❌ |

**決策理由：** 應用狀態簡單，不需要框架；Vanilla JS + Tailwind 開發速度更快。

#### 部署：Render vs Heroku vs AWS

| 平台 | 優點 | 缺點 | 選擇 |
|-----|------|------|-----|
| **Render** | 免費 HTTPS、自動部署 | 冷啟動時間長 | ✅ 選擇 |
| Heroku | 老牌穩定 | 免費方案取消 | ❌ |
| AWS | 強大靈活 | 配置複雜、成本高 | ❌ |

**決策理由：** 免費、簡單、HTTPS，適合個人專案。

---

## 第二章：技術架構深度解析

### 2.1 整體架構圖

```
┌─────────────────────────────────────────────────────────┐
│                    用戶瀏覽器 (Browser)                    │
│  ┌─────────────────────────────────────────────────┐   │
│  │  前端 UI (HTML + Tailwind CSS + Vanilla JS)     │   │
│  │  - 格式選擇器                                     │   │
│  │  - 文獻輸入框                                     │   │
│  │  - 結果顯示表格                                   │   │
│  │  - 匯出按鈕                                       │   │
│  └─────────────────────────────────────────────────┘   │
└──────────────────────┬──────────────────────────────────┘
                       │ HTTP/HTTPS (AJAX)
                       ▼
┌─────────────────────────────────────────────────────────┐
│              Flask Web Server (Gunicorn)                │
│  ┌───────────────────────────────────────────────────┐ │
│  │  路由層 (app.py)                                   │ │
│  │  - GET  /           → 主頁面                      │ │
│  │  - POST /parse      → 解析文獻                    │ │
│  │  - POST /export/... → 匯出文檔                    │ │
│  │  - GET  /health     → 健康檢查                    │ │
│  └───────────────────────────────────────────────────┘ │
│  ┌───────────────────────────────────────────────────┐ │
│  │  業務邏輯層 (modules/)                             │ │
│  │  ┌─────────────┐ ┌──────────────┐ ┌───────────┐ │ │
│  │  │ Parser      │ │ API Client   │ │ Formatter │ │ │
│  │  │ (解析器)     │ │ (API客戶端)   │ │ (格式化器) │ │ │
│  │  └─────────────┘ └──────────────┘ └───────────┘ │ │
│  └───────────────────────────────────────────────────┘ │
└──────────────────────┬──────────────────────────────────┘
                       │ HTTP Request
                       ▼
┌─────────────────────────────────────────────────────────┐
│              外部 API (CrossRef)                         │
│  - 透過 DOI 查詢完整文獻資料                              │
│  - 透過標題/作者模糊查詢                                  │
└─────────────────────────────────────────────────────────┘
```

---

### 2.2 資料流分析

#### 用戶解析文獻的完整流程

```
1. 用戶輸入文獻
   ↓
2. 前端發送 AJAX POST 到 /parse
   {
     "text": "Smith, J. (2020). Article title...",
     "format": "apa",
     "enrich": true
   }
   ↓
3. Flask 接收請求 → 按行分割文獻
   ↓
4. 對每條文獻：
   ├─ Parser 解析 → 提取 authors, year, title, DOI...
   ├─ API Client 補完 → 透過 DOI/標題查詢 CrossRef
   └─ Formatter 格式化 → 生成 APA/MLA/Chicago/Harvard 版本
   ↓
5. 返回 JSON 結果
   {
     "success": true,
     "references": [
       {
         "id": 0,
         "original": "原始輸入",
         "formatted_all": {
           "apa": "Smith, J. (2020)...",
           "mla": "Smith, John...",
           ...
         },
         "data": { authors: [...], year: "2020", ... }
       }
     ]
   }
   ↓
6. 前端渲染結果表格
```

---

### 2.3 檔案結構詳解

```
reference-formatter/
│
├── app.py                    # Flask 主應用（路由定義）
│   └── 職責：處理 HTTP 請求，協調各模組工作
│
├── wsgi.py                   # WSGI 入口（生產環境）
│   └── 職責：Gunicorn 啟動入口
│
├── config.py                 # 配置管理（環境分離）
│   └── 職責：開發/生產環境配置
│
├── requirements.txt          # Python 依賴清單
│   └── 內容：Flask, requests, python-docx, pybtex, gunicorn
│
├── Dockerfile                # Docker 容器定義
│   └── 職責：標準化部署環境
│
├── render.yaml               # Render 部署配置
│   └── 職責：自動化部署流程
│
├── gunicorn.conf.py          # Gunicorn 配置
│   └── 職責：生產環境 WSGI 伺服器設定
│
├── modules/                  # 業務邏輯模組
│   ├── __init__.py
│   ├── parser.py             # 文獻解析器（正則表達式）
│   │   └── class ReferenceParser
│   ├── api_client.py         # API 客戶端（CrossRef 整合）
│   │   └── class APIClient
│   └── formatter.py          # 格式化器（APA/MLA/Chicago/Harvard）
│       └── class ReferenceFormatter + 4 個格式化子類
│
├── templates/                # Jinja2 HTML 模板
│   └── index.html            # 主頁面（SPA 單頁應用）
│
├── static/                   # 靜態資源
│   ├── css/
│   │   └── style.css         # 自定義 CSS（補充 Tailwind）
│   └── js/
│       └── main.js           # 前端邏輯（AJAX、DOM 操作）
│
├── data/                     # 資料目錄（預留）
│
└── docs/                     # 文檔目錄
    ├── DEPLOYMENT.md         # 部署指南
    ├── SECURITY.md           # 安全性說明
    └── PERFORMANCE.md        # 性能優化文檔
```

---

## 第三章：核心模組開發實戰

### 3.1 解析引擎 (Parser) - 從文本到結構化資料

#### 3.1.1 核心挑戰

**輸入：** 非結構化的文獻文本
```
Smith, J. (2020). The impact of AI on education. Nature, 582(7812), 123-145. https://doi.org/10.1038/s41586-020-1234-5
```

**輸出：** 結構化的字典
```python
{
    'authors': [{'last': 'Smith', 'first': 'J'}],
    'year': '2020',
    'title': 'The impact of AI on education',
    'journal': 'Nature',
    'volume': '582',
    'issue': '7812',
    'pages': '123-145',
    'doi': '10.1038/s41586-020-1234-5',
    'type': 'article',
    'completeness': 1.0,
    'confidence': 0.9
}
```

---

#### 3.1.2 正則表達式設計

**檔案位置：** `modules/parser.py`

##### DOI 提取

```python
# DOI 格式：10.xxxx/xxxxx
DOI_PATTERN = r'10\.\d{4,9}/[-._;()/:A-Za-z0-9]+'

def extract_doi(self, text: str) -> Optional[str]:
    """提取 DOI"""
    # 1. 移除常見前綴
    text = text.replace('doi:', '').replace('DOI:', '')
    text = text.replace('https://doi.org/', '')

    # 2. 正則匹配
    match = re.search(self.DOI_PATTERN, text, re.IGNORECASE)
    if match:
        doi = match.group(0)
        # 3. 清理尾部標點符號（常見錯誤）
        doi = doi.rstrip('.,;')
        return doi
    return None

# 測試案例
# 輸入：「10.1038/nature12373.」（末尾有句點）
# 輸出：「10.1038/nature12373」（正確清理）
```

**學習重點：**
- `re.IGNORECASE`：忽略大小寫
- `rstrip('.,;')`：移除右側標點符號
- 防禦性編程：處理各種邊界情況

---

##### 年份提取

```python
# 年份範圍：1900-2099
YEAR_PATTERN = r'\b(19\d{2}|20\d{2})\b'

def extract_year(self, text: str) -> Optional[str]:
    """提取年份（優先括號內）"""
    # 1. 優先匹配括號中的年份：(2020)
    year_match = re.search(r'\((' + self.YEAR_PATTERN + r')\)', text)
    if year_match:
        return year_match.group(1)  # 返回括號內的年份

    # 2. 如果沒有括號，找任何年份
    year_match = re.search(self.YEAR_PATTERN, text)
    if year_match:
        return year_match.group(0)

    return None

# 測試案例
# 輸入：「Smith (2020). Article published in 1995...」
# 輸出：「2020」（優先選擇括號內的）
```

**學習重點：**
- `\b`：單詞邊界，避免匹配「12020」這樣的數字
- 優先級設計：括號內 > 任意位置

---

##### 作者提取

```python
def extract_authors(self, text: str) -> List[Dict[str, str]]:
    """提取作者姓名"""
    authors = []

    # 1. 嘗試匹配 APA 格式：Last, F. M.
    apa_pattern = r'([A-Z][a-z]+(?:-[A-Z][a-z]+)?),\s*([A-Z]\.(?:\s*[A-Z]\.)?)'
    matches = re.findall(apa_pattern, text)

    for match in matches:
        authors.append({
            'last': match[0],                          # 姓
            'first': match[1].replace('.', '').replace(' ', '')  # 名（去掉點和空格）
        })

    # 2. 如果沒找到，嘗試簡單格式：FirstName LastName
    if not authors:
        simple_pattern = r'\b([A-Z][a-z]+)\s+([A-Z][a-z]+)\b'
        matches = re.findall(simple_pattern, text[:100])  # 只搜尋前 100 字元

        for match in matches[:3]:  # 最多取 3 個（避免誤匹配）
            authors.append({
                'last': match[1],
                'first': match[0][0]  # 只取首字母
            })

    return authors

# 測試案例 1
# 輸入：「Smith, J. A., Johnson, M., & Lee, K. (2020)...」
# 輸出：[
#   {'last': 'Smith', 'first': 'JA'},
#   {'last': 'Johnson', 'first': 'M'},
#   {'last': 'Lee', 'first': 'K'}
# ]

# 測試案例 2（容錯）
# 輸入：「John Smith and Mary Johnson wrote...」
# 輸出：[
#   {'last': 'Smith', 'first': 'J'},
#   {'last': 'Johnson', 'first': 'M'}
# ]
```

**學習重點：**
- `(?:...)`：非捕獲組（不會影響 group 編號）
- `(?:-[A-Z][a-z]+)?`：可選的連字符姓氏（如 Smith-Jones）
- 雙重策略：先嘗試精確匹配，失敗則降級到模糊匹配

---

#### 3.1.3 完整度與信心度計算

```python
def calculate_completeness(self, data: Dict) -> float:
    """計算資料完整度（0.0 - 1.0）"""
    required_fields = {
        'article': ['authors', 'year', 'title', 'journal'],
        'book': ['authors', 'year', 'title', 'publisher'],
        'website': ['title', 'url'],
        'unknown': ['authors', 'year', 'title']
    }

    ref_type = data.get('type', 'unknown')
    fields = required_fields.get(ref_type, required_fields['unknown'])

    present = 0
    for field in fields:
        value = data.get(field)
        # 檢查：不為 None，且如果是列表則非空
        if value and (not isinstance(value, list) or len(value) > 0):
            present += 1

    return present / len(fields) if fields else 0.0

# 範例
# 文章有 authors, year, title 但沒有 journal
# completeness = 3/4 = 0.75


def calculate_confidence(self, data: Dict) -> float:
    """計算解析信心度（0.0 - 1.0）"""
    confidence = 0.0

    # DOI 存在：+0.4（最可靠）
    if data.get('doi'):
        confidence += 0.4

    # 作者存在且格式良好：+0.2
    if data.get('authors') and len(data['authors']) > 0:
        confidence += 0.2

    # 年份存在：+0.1
    if data.get('year'):
        confidence += 0.1

    # 標題存在：+0.1
    if data.get('title'):
        confidence += 0.1

    # 期刊資訊存在：+0.2
    if data.get('journal') or data.get('volume'):
        confidence += 0.2

    return min(confidence, 1.0)  # 最高 1.0

# 範例
# 有 DOI + authors + year + title + journal
# confidence = 0.4 + 0.2 + 0.1 + 0.1 + 0.2 = 1.0
```

**設計思想：**
- **完整度**：衡量必要欄位的覆蓋率
- **信心度**：衡量解析結果的可靠性
- 用途：前端可以顯示警告（如「信心度 < 0.5，建議手動檢查」）

---

### 3.2 API 客戶端 (APIClient) - 資料補完

#### 3.2.1 為什麼需要 API？

**解析器的限制：**
- 無法從不完整的文本推斷缺失資訊
- 無法驗證解析結果的正確性
- 無法處理僅有 DOI 或標題的情況

**CrossRef API 的優勢：**
- 擁有海量學術文獻元數據
- 透過 DOI 精確查詢
- 透過標題/作者模糊匹配
- **免費且無需 API Key**（禮貌性提供 email 即可）

---

#### 3.2.2 CrossRef API 整合

**檔案位置：** `modules/api_client.py`

##### 透過 DOI 查詢

```python
CROSSREF_API = "https://api.crossref.org/works"

def query_by_doi(self, doi: str) -> Optional[Dict]:
    """通過 DOI 查詢 CrossRef"""
    if not doi:
        return None

    url = f"{self.CROSSREF_API}/{doi}"

    try:
        response = self._make_request(url)
        if response and response.get('status') == 'ok':
            # CrossRef 回應格式：
            # {
            #   "status": "ok",
            #   "message": { ... 文獻元數據 ... }
            # }
            return self._parse_crossref_response(response['message'])
    except Exception as e:
        logger.error(f"CrossRef 查詢失敗 (DOI: {doi}): {e}")

    return None

# API 請求範例
# GET https://api.crossref.org/works/10.1038/nature12373
# 返回：Nature 雜誌上的某篇文章的完整元數據
```

---

##### 透過標題查詢（模糊匹配）

```python
def query_by_metadata(self, title: str, authors: List[str] = None) -> Optional[Dict]:
    """通過標題和作者模糊查詢"""
    if not title:
        return None

    # 構建查詢字串
    query_parts = [f'title:"{title}"']
    if authors:
        author_query = ' '.join(authors[:2])  # 只用前兩個作者
        query_parts.append(f'author:"{author_query}"')

    query = ' '.join(query_parts)
    url = f"{self.CROSSREF_API}?query={query}&rows=1"  # 只取第一筆結果

    try:
        response = self._make_request(url)
        if response and response.get('status') == 'ok':
            items = response['message'].get('items', [])
            if items:
                return self._parse_crossref_response(items[0])
    except Exception as e:
        logger.error(f"標題查詢失敗: {e}")

    return None

# API 請求範例
# GET https://api.crossref.org/works?query=title:"machine learning"&rows=1
# 返回：標題最匹配的文章
```

---

#### 3.2.3 重試機制與錯誤處理

```python
def _make_request(self, url: str) -> Optional[Dict]:
    """發送 HTTP 請求（含重試機制）"""
    for attempt in range(self.max_retries):  # 預設 3 次
        try:
            response = requests.get(
                url,
                headers=self.HEADERS,  # User-Agent 禮貌性標識
                timeout=self.timeout   # 預設 10 秒
            )

            # 成功
            if response.status_code == 200:
                return response.json()

            # 資源不存在（不需重試）
            elif response.status_code == 404:
                logger.warning(f"資源不存在 (404): {url}")
                return None

            # 速率限制（需重試）
            elif response.status_code == 429:
                wait_time = 2 ** attempt  # 指數退避：1s, 2s, 4s...
                logger.warning(f"速率限制，等待 {wait_time} 秒後重試...")
                time.sleep(wait_time)

            # 其他錯誤
            else:
                logger.warning(f"HTTP {response.status_code}: {url}")

        except requests.Timeout:
            logger.warning(f"請求超時 (嘗試 {attempt + 1}/{self.max_retries})")
        except Exception as e:
            logger.error(f"請求失敗: {e}")
            break  # 非網路錯誤，不重試

    return None
```

**學習重點：**
- **指數退避**：避免頻繁重試加劇伺服器負擔
- **區分錯誤類型**：404 不重試，429 重試，其他錯誤視情況
- **超時設定**：防止長時間等待阻塞

---

#### 3.2.4 CrossRef 回應解析

```python
def _parse_crossref_response(self, data: Dict) -> Dict:
    """解析 CrossRef API 回應"""
    result = {
        'title': None,
        'authors': [],
        'year': None,
        'journal': None,
        'volume': None,
        'issue': None,
        'pages': None,
        'doi': None,
        'publisher': None,
        'type': 'article'
    }

    # 標題（可能是陣列）
    if 'title' in data and data['title']:
        result['title'] = data['title'][0]

    # 作者
    if 'author' in data:
        for author in data['author']:
            result['authors'].append({
                'last': author.get('family', ''),
                'first': author.get('given', '')
            })

    # 年份（優先紙本出版日期）
    if 'published-print' in data:
        date_parts = data['published-print'].get('date-parts', [[]])
        if date_parts and date_parts[0]:
            result['year'] = str(date_parts[0][0])  # 第一個元素是年份
    elif 'published-online' in data:
        date_parts = data['published-online'].get('date-parts', [[]])
        if date_parts and date_parts[0]:
            result['year'] = str(date_parts[0][0])

    # 期刊名稱（container-title 可能是陣列）
    if 'container-title' in data and data['container-title']:
        result['journal'] = data['container-title'][0]

    # 卷號、期號、頁碼
    result['volume'] = data.get('volume')
    result['issue'] = data.get('issue')
    result['pages'] = data.get('page')

    # DOI
    result['doi'] = data.get('DOI')

    # 出版商
    result['publisher'] = data.get('publisher')

    return result
```

**CrossRef API 回應範例：**
```json
{
  "title": ["Article title here"],
  "author": [
    {"family": "Smith", "given": "John"},
    {"family": "Doe", "given": "Jane"}
  ],
  "published-print": {
    "date-parts": [[2020, 3, 15]]
  },
  "container-title": ["Nature"],
  "volume": "582",
  "issue": "7812",
  "page": "123-145",
  "DOI": "10.1038/nature12373"
}
```

---

### 3.3 格式化引擎 (Formatter) - 統一資料到多格式

#### 3.3.1 設計模式：策略模式 + 模板方法

**檔案位置：** `modules/formatter.py`

##### 抽象基類設計

```python
from abc import ABC, abstractmethod

class BaseFormatter(ABC):
    """文獻格式化器基類"""

    @abstractmethod
    def format_article(self, data: Dict) -> str:
        """格式化期刊文章（必須實現）"""
        pass

    @abstractmethod
    def format_book(self, data: Dict) -> str:
        """格式化書籍（必須實現）"""
        pass

    @abstractmethod
    def format_website(self, data: Dict) -> str:
        """格式化網站（必須實現）"""
        pass

    def format(self, data: Dict) -> str:
        """根據文獻類型自動選擇格式化方法（模板方法）"""
        ref_type = data.get('type', 'unknown')

        if ref_type == 'article':
            return self.format_article(data)
        elif ref_type == 'book':
            return self.format_book(data)
        elif ref_type == 'website':
            return self.format_website(data)
        else:
            # 未知類型，嘗試作為文章格式化
            return self.format_article(data)

    def _format_authors(self, authors: List[Dict],
                       max_authors: int = None,
                       last_first: bool = True,
                       ampersand: bool = False,
                       invert_all: bool = False) -> str:
        """
        格式化作者列表（模板方法，被子類共用）

        Args:
            authors: 作者列表
            max_authors: 最多顯示作者數（超過則 et al.）
            last_first: 是否使用「姓, 名」格式
            ampersand: 最後一個作者前用 & 還是 and
            invert_all: 是否所有作者都倒置（姓在前）
        """
        if not authors:
            return ""

        # 限制作者數量
        if max_authors and len(authors) > max_authors:
            authors = authors[:max_authors]
            use_et_al = True
        else:
            use_et_al = False

        formatted = []
        for i, author in enumerate(authors):
            last = author.get('last', '')
            first = author.get('first', '')

            # 第一個作者或所有作者都倒置
            if (i == 0 and last_first) or (invert_all and last_first):
                if first:
                    formatted.append(f"{last}, {first}.")
                else:
                    formatted.append(last)
            else:
                if first:
                    formatted.append(f"{first}. {last}")
                else:
                    formatted.append(last)

        # 組合作者
        if len(formatted) == 1:
            result = formatted[0]
        elif len(formatted) == 2:
            connector = " & " if ampersand else " and "
            result = connector.join(formatted)
        else:
            connector = ", & " if ampersand else ", and "
            result = ", ".join(formatted[:-1]) + connector + formatted[-1]

        if use_et_al:
            result += " et al."

        return result
```

**設計亮點：**
- `ABC` 抽象基類：定義介面契約，強制子類實現
- `_format_authors` 模板方法：避免重複代碼
- 參數化設計：同一方法適配不同格式需求

---

#### 3.3.2 APA 7th Edition 格式化器

```python
class APAFormatter(BaseFormatter):
    """APA 7th Edition 格式化器"""

    def format_article(self, data: Dict) -> str:
        """
        APA 期刊文章格式：
        Author, A. A., & Author, B. B. (Year). Title of article.
        Title of Periodical, volume(issue), pages. https://doi.org/xxx
        """
        parts = []

        # 1. 作者（姓, 名縮寫，所有作者都倒置，用 & 連接）
        authors = self._format_authors(
            data.get('authors', []),
            max_authors=20,      # APA 允許最多 20 個作者
            last_first=True,     # 姓在前
            ampersand=True,      # 使用 &
            invert_all=True      # 所有作者都倒置
        )
        if authors:
            parts.append(authors)

        # 2. 年份（n.d. = no date）
        year = data.get('year', 'n.d.')
        parts.append(f"({year}).")

        # 3. 標題（不使用斜體，句首大寫）
        title = data.get('title', 'Untitled')
        parts.append(f"{title}.")

        # 4. 期刊名稱（斜體，Title Case）
        journal = data.get('journal', '')
        if journal:
            journal_part = f"*{journal}*"

            # 卷號（斜體）
            volume = data.get('volume', '')
            if volume:
                journal_part += f", *{volume}*"

            # 期號（不斜體，括號內）
            issue = data.get('issue', '')
            if issue:
                journal_part += f"({issue})"

            # 頁碼
            pages = data.get('pages', '')
            if pages:
                journal_part += f", {pages}"

            parts.append(journal_part + ".")

        # 5. DOI 或 URL
        doi = data.get('doi', '')
        if doi:
            parts.append(f"https://doi.org/{doi}")
        elif data.get('url'):
            parts.append(data['url'])

        return " ".join(parts)

    # 範例輸出：
    # Smith, J. A., & Johnson, M. (2020). The impact of AI on education.
    # *Nature*, *582*(7812), 123-145. https://doi.org/10.1038/nature12373
```

---

#### 3.3.3 MLA 9th Edition 格式化器

```python
class MLAFormatter(BaseFormatter):
    """MLA 9th Edition 格式化器"""

    def format_article(self, data: Dict) -> str:
        """
        MLA 期刊文章格式：
        Author Last, First. "Title of Article." Title of Journal,
        vol. #, no. #, Year, pp. #-#.
        """
        parts = []

        # 1. 作者（只有第一個作者倒置，用 and 連接，最多 3 個）
        authors = self._format_authors(
            data.get('authors', []),
            max_authors=3,       # MLA 超過 3 個用 et al.
            last_first=True,
            ampersand=False,     # 使用 and
            invert_all=False     # 只有第一個作者倒置
        )
        if authors:
            parts.append(authors + ".")

        # 2. 標題（雙引號，Title Case）
        title = data.get('title', 'Untitled')
        parts.append(f'"{title}."')

        # 3. 期刊名稱（斜體）
        journal = data.get('journal', '')
        if journal:
            journal_part = f"*{journal}*"

            # 卷號（vol. #）
            volume = data.get('volume', '')
            if volume:
                journal_part += f", vol. {volume}"

            # 期號（no. #）
            issue = data.get('issue', '')
            if issue:
                journal_part += f", no. {issue}"

            parts.append(journal_part + ",")

        # 4. 年份
        year = data.get('year', 'n.d.')
        parts.append(f"{year},")

        # 5. 頁碼（pp. #-#）
        pages = data.get('pages', '')
        if pages:
            parts.append(f"pp. {pages}.")
        else:
            # 移除最後的逗號
            if parts and parts[-1].endswith(','):
                parts[-1] = parts[-1][:-1] + '.'

        return " ".join(parts)

    # 範例輸出：
    # Smith, John A., and Mary Johnson. "The Impact of AI on Education."
    # *Nature*, vol. 582, no. 7812, 2020, pp. 123-145.
```

---

#### 3.3.4 格式化器管理類

```python
class ReferenceFormatter:
    """文獻格式化器管理類（工廠模式）"""

    # 註冊所有格式化器
    FORMATTERS = {
        'apa': APAFormatter(),
        'mla': MLAFormatter(),
        'chicago': ChicagoFormatter(),
        'harvard': HarvardFormatter(),
    }

    @classmethod
    def format(cls, data: Dict, style: str = 'apa') -> str:
        """格式化文獻"""
        style = style.lower()
        formatter = cls.FORMATTERS.get(style)

        if not formatter:
            raise ValueError(f"不支援的格式: {style}")

        return formatter.format(data)

    @classmethod
    def get_available_styles(cls) -> List[str]:
        """獲取所有可用的格式"""
        return list(cls.FORMATTERS.keys())

# 使用範例
formatted_apa = ReferenceFormatter.format(data, 'apa')
formatted_mla = ReferenceFormatter.format(data, 'mla')
```

---

## 第四章：Flask 應用開發

### 4.1 應用結構與配置

#### 4.1.1 配置管理（環境分離）

**檔案：** `config.py`

```python
import os
from datetime import timedelta

class Config:
    """基礎配置（共用設定）"""

    # 安全性
    SECRET_KEY = os.environ.get('SECRET_KEY') or 'dev-secret-key-please-change-in-production'

    # 文件上傳限制
    MAX_CONTENT_LENGTH = 16 * 1024 * 1024  # 16MB

    # Session 設定
    PERMANENT_SESSION_LIFETIME = timedelta(hours=24)
    SESSION_COOKIE_SECURE = True   # 僅 HTTPS
    SESSION_COOKIE_HTTPONLY = True # 防 XSS
    SESSION_COOKIE_SAMESITE = 'Lax'

    # API 配置
    CROSSREF_EMAIL = os.environ.get('CROSSREF_EMAIL', 'support@example.com')

    # 日誌
    LOG_LEVEL = os.environ.get('LOG_LEVEL', 'INFO')

class DevelopmentConfig(Config):
    """開發環境配置"""
    DEBUG = True
    SESSION_COOKIE_SECURE = False  # 開發環境可用 HTTP

class ProductionConfig(Config):
    """生產環境配置"""
    DEBUG = False
    TESTING = False
    # 繼承基類的安全設定

class TestingConfig(Config):
    """測試環境配置"""
    TESTING = True
    WTF_CSRF_ENABLED = False  # 測試時關閉 CSRF

# 配置字典（工廠模式）
config = {
    'development': DevelopmentConfig,
    'production': ProductionConfig,
    'testing': TestingConfig,
    'default': DevelopmentConfig
}
```

**設計原則：**
- 繼承：共用配置在基類，特殊配置在子類
- 環境變數：敏感資訊（SECRET_KEY）從環境變數讀取
- 安全預設：生產環境強制 HTTPS、HttpOnly Cookie

---

#### 4.1.2 應用初始化

**檔案：** `app.py`

```python
from flask import Flask, render_template, request, jsonify, send_file
import os
from modules.parser import ReferenceParser
from modules.api_client import APIClient
from modules.formatter import ReferenceFormatter
from config import config

# 創建應用實例
app = Flask(__name__)

# 載入配置
env = os.environ.get('FLASK_ENV', 'development')
app.config.from_object(config[env])

# 生產環境安全檢查
if env == 'production':
    if not app.config.get('SECRET_KEY') or \
       app.config['SECRET_KEY'] == 'dev-secret-key-please-change-in-production':
        raise ValueError("生產環境必須設定有效的 SECRET_KEY 環境變量！")

# 初始化模組（單例模式）
parser = ReferenceParser()
api_client = APIClient()

# 路由定義...
```

**重要細節：**
- `FLASK_ENV` 環境變數控制配置
- 生產環境安全檢查：防止使用預設密鑰
- 模組單例：避免重複創建

---

### 4.2 路由設計與實現

#### 4.2.1 主頁面路由

```python
@app.route('/')
def index():
    """主頁面（SPA 單頁應用）"""
    return render_template('index.html')
```

簡單但重要：前端是 SPA，所有交互透過 AJAX。

---

#### 4.2.2 健康檢查端點

```python
@app.route('/health')
def health_check():
    """健康檢查端點（用於 Render 監控）"""
    return jsonify({
        'status': 'healthy',
        'service': 'reference-formatter',
        'version': '1.0.0'
    }), 200
```

**用途：**
- Render 平台定期請求此端點
- 返回非 200 狀態碼會觸發重啟
- 可擴展為檢查資料庫連線等

---

#### 4.2.3 文獻解析端點（核心）

```python
@app.route('/parse', methods=['POST'])
def parse_references():
    """
    解析文獻端點

    請求格式：
    {
      "text": "多行文獻文本",
      "format": "apa",
      "enrich": true
    }

    回應格式：
    {
      "success": true,
      "count": 2,
      "references": [
        {
          "id": 0,
          "original": "原始輸入",
          "formatted": "APA 格式化結果",
          "formatted_all": {
            "apa": "...",
            "mla": "...",
            "chicago": "...",
            "harvard": "..."
          },
          "data": { authors: [...], year: "2020", ... },
          "completeness": 0.95,
          "confidence": 0.8
        }
      ]
    }
    """
    try:
        # 1. 解析請求
        data = request.get_json()
        raw_text = data.get('text', '')
        format_style = data.get('format', 'apa')
        enrich = data.get('enrich', True)

        # 2. 分割文獻（每行一條）
        lines = [line.strip() for line in raw_text.split('\n') if line.strip()]

        # 3. 處理每條文獻
        results = []
        for i, line in enumerate(lines):
            # 3.1 解析文獻
            parsed_data = parser.parse_reference(line)

            # 3.2 API 補完資料（可選）
            if enrich and (parsed_data.get('doi') or parsed_data.get('title')):
                try:
                    enriched_data = api_client.enrich_reference(parsed_data)
                    if enriched_data.get('enriched'):
                        parsed_data = enriched_data
                        status = 'enriched'
                    else:
                        status = 'complete'
                except Exception as e:
                    print(f"API 查詢失敗: {e}")
                    status = 'complete'
            else:
                status = 'complete'

            # 3.3 格式化為所有格式（前端可切換）
            formatted_refs = {}
            for style in ReferenceFormatter.get_available_styles():
                try:
                    formatted_refs[style] = ReferenceFormatter.format(parsed_data, style)
                except Exception as e:
                    formatted_refs[style] = f"[格式化失敗: {str(e)}]"

            # 3.4 組合結果
            results.append({
                'id': i,
                'original': line,
                'status': status,
                'formatted': formatted_refs.get(format_style, formatted_refs['apa']),
                'formatted_all': formatted_refs,
                'data': parsed_data,
                'completeness': parsed_data.get('completeness', 0.0),
                'confidence': parsed_data.get('confidence', 0.0)
            })

        # 4. 返回結果
        return jsonify({
            'success': True,
            'count': len(results),
            'references': results,
            'available_formats': ReferenceFormatter.get_available_styles()
        })

    except Exception as e:
        # 錯誤處理
        import traceback
        traceback.print_exc()  # 印出完整錯誤堆疊（開發環境）
        return jsonify({
            'success': False,
            'error': str(e)
        }), 400
```

**設計亮點：**
- 一次解析，生成所有格式（避免重複解析）
- 優雅降級：API 失敗不影響基本功能
- 詳細錯誤資訊：`traceback.print_exc()` 輔助除錯

---

#### 4.2.4 匯出端點（DOCX 範例）

```python
@app.route('/export/docx', methods=['POST'])
def export_docx():
    """
    匯出為 Word 文檔

    請求格式：
    {
      "references": [
        {"formatted": "Smith, J. (2020)..."},
        ...
      ],
      "style": "apa"
    }
    """
    try:
        data = request.get_json()
        references = data.get('references', [])
        citation_style = data.get('style', 'apa')

        # 1. 創建 Word 文檔
        from docx import Document
        from docx.shared import Pt
        from docx.enum.text import WD_ALIGN_PARAGRAPH

        doc = Document()

        # 2. 添加標題
        doc.add_heading('參考文獻 / References', 0)

        # 3. 添加格式資訊
        style_names = {
            'apa': 'APA 7th Edition',
            'mla': 'MLA 9th Edition',
            'chicago': 'Chicago Manual of Style 17th Edition',
            'harvard': 'Harvard Referencing Style'
        }
        style_para = doc.add_paragraph(f'格式:{style_names.get(citation_style, citation_style.upper())}')
        style_para.alignment = WD_ALIGN_PARAGRAPH.CENTER

        doc.add_paragraph()  # 空行

        # 4. 添加每條文獻（懸掛縮排）
        for ref in references:
            p = doc.add_paragraph(ref.get('formatted', ''))
            p.paragraph_format.left_indent = Pt(36)        # 左縮排 36pt
            p.paragraph_format.first_line_indent = Pt(-36) # 首行負縮排（懸掛）

        # 5. 保存到記憶體（不寫入磁碟）
        buffer = io.BytesIO()
        doc.save(buffer)
        buffer.seek(0)  # 重置指針到開頭

        # 6. 返回檔案
        return send_file(
            buffer,
            mimetype='application/vnd.openxmlformats-officedocument.wordprocessingml.document',
            as_attachment=True,
            download_name=f'references_{citation_style}.docx'
        )

    except Exception as e:
        import traceback
        traceback.print_exc()
        return jsonify({'error': str(e)}), 400
```

**技術要點：**
- `BytesIO`：記憶體檔案，避免磁碟 I/O
- 懸掛縮排：專業文獻格式要求
- `send_file`：Flask 內建檔案下載

---

### 4.3 WSGI 生產環境入口

**檔案：** `wsgi.py`

```python
"""
WSGI 入口文件
用於生產環境部署（Gunicorn）
"""

from app import app

if __name__ == "__main__":
    app.run()
```

**用途：**
```bash
# 開發環境
python app.py

# 生產環境
gunicorn --config gunicorn.conf.py wsgi:app
```

---

## 第五章：前端交互設計

### 5.1 HTML 結構設計

**檔案：** `templates/index.html`

#### 5.1.1 頁面結構

```html
<!DOCTYPE html>
<html lang="zh-TW">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>學術文獻格式整理工具</title>

    <!-- Tailwind CSS CDN（快速開發）-->
    <script src="https://cdn.tailwindcss.com"></script>

    <!-- 自定義 CSS -->
    <link rel="stylesheet" href="{{ url_for('static', filename='css/style.css') }}">
</head>
<body class="bg-gray-50 min-h-screen">
    <div class="container mx-auto px-4 py-8 max-w-6xl">
        <!-- 標題區 -->
        <header class="text-center mb-8">
            <h1 class="text-4xl font-bold text-gray-800 mb-2">
                學術文獻格式整理工具
            </h1>
            <p class="text-gray-600">
                支援 APA、MLA、Chicago、Harvard 等多種文獻格式
            </p>
        </header>

        <!-- 主要功能區 -->
        <div class="bg-white rounded-lg shadow-lg p-6 mb-6">
            <!-- 格式選擇器 -->
            <div class="mb-6">
                <label class="block text-lg font-semibold text-gray-700 mb-3">
                    選擇文獻格式
                </label>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <button class="format-btn active" data-format="apa">
                        APA 7th
                    </button>
                    <button class="format-btn" data-format="mla">
                        MLA 9th
                    </button>
                    <button class="format-btn" data-format="chicago">
                        Chicago 17th
                    </button>
                    <button class="format-btn" data-format="harvard">
                        Harvard
                    </button>
                </div>
            </div>

            <!-- 輸入區域 -->
            <div class="mb-6">
                <label for="input-text" class="block text-lg font-semibold text-gray-700 mb-2">
                    輸入文獻（每行一條）
                </label>
                <textarea
                    id="input-text"
                    rows="10"
                    class="w-full p-4 border border-gray-300 rounded-lg"
                    placeholder="範例：&#10;Smith, J. (2020). Article title. Journal Name, 10(2), 123-145."
                ></textarea>
            </div>

            <!-- 按鈕區域 -->
            <div class="flex gap-4 mb-6">
                <button id="parse-btn" class="btn-primary">
                    解析並格式化
                </button>
                <button id="clear-btn" class="btn-secondary">
                    清除
                </button>
            </div>

            <!-- 載入中提示 -->
            <div id="loading" class="hidden text-center py-8">
                <div class="spinner"></div>
                <p class="text-gray-600 mt-4">正在處理中...</p>
            </div>
        </div>

        <!-- 結果區域（動態顯示）-->
        <div id="results-container" class="hidden">
            <!-- JavaScript 動態插入結果 -->
        </div>
    </div>

    <!-- JavaScript -->
    <script src="{{ url_for('static', filename='js/main.js') }}"></script>
</body>
</html>
```

**設計要點：**
- Tailwind CDN：快速原型開發
- 語義化 HTML：清晰的結構
- 響應式設計：`grid-cols-2 md:grid-cols-4`

---

### 5.2 JavaScript 交互邏輯

**檔案：** `static/js/main.js`

#### 5.2.1 全域狀態管理

```javascript
// 全域狀態
let processedReferences = [];  // 已處理的文獻
let currentFormat = 'apa';     // 當前選擇的格式

// DOM 元素
const inputText = document.getElementById('input-text');
const parseBtn = document.getElementById('parse-btn');
const clearBtn = document.getElementById('clear-btn');
const loading = document.getElementById('loading');
const resultsContainer = document.getElementById('results-container');
const formatBtns = document.querySelectorAll('.format-btn');
```

---

#### 5.2.2 格式切換功能

```javascript
// 格式選擇按鈕點擊事件
formatBtns.forEach(btn => {
    btn.addEventListener('click', () => {
        const format = btn.dataset.format;  // data-format 屬性

        // 1. 更新按鈕樣式
        formatBtns.forEach(b => {
            b.classList.remove('active', 'bg-blue-600', 'text-white');
            b.classList.add('bg-gray-200', 'text-gray-800');
        });
        btn.classList.remove('bg-gray-200', 'text-gray-800');
        btn.classList.add('active', 'bg-blue-600', 'text-white');

        // 2. 更新當前格式
        currentFormat = format;

        // 3. 如果已有結果，更新顯示（無需重新解析）
        if (processedReferences.length > 0) {
            updateReferencesFormat(format);
        }
    });
});

function updateReferencesFormat(format) {
    """切換格式時更新結果顯示"""
    const formatNames = {
        'apa': 'APA 7th',
        'mla': 'MLA 9th',
        'chicago': 'Chicago 17th',
        'harvard': 'Harvard'
    };

    // 更新表格標題
    document.getElementById('format-display').textContent = formatNames[format];

    // 更新每行的格式化結果
    processedReferences.forEach((ref, index) => {
        const cell = document.querySelector(`#ref-${index} .formatted-cell`);
        if (cell) {
            cell.textContent = ref.formatted_all[format];
        }
    });
}
```

**關鍵點：**
- 格式預存：一次解析，生成所有格式
- 切換即時：無需重新請求後端
- 狀態同步：`currentFormat` 記錄當前選擇

---

#### 5.2.3 AJAX 請求處理

```javascript
parseBtn.addEventListener('click', async () => {
    const text = inputText.value.trim();

    // 驗證輸入
    if (!text) {
        alert('請輸入文獻資料');
        return;
    }

    // 顯示載入中
    loading.classList.remove('hidden');
    resultsContainer.classList.add('hidden');
    parseBtn.disabled = true;

    try {
        // 發送 AJAX 請求
        const response = await fetch('/parse', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({
                text: text,
                format: currentFormat,
                enrich: true  // 啟用 API 補完
            })
        });

        // 解析回應
        const data = await response.json();

        if (data.success) {
            // 保存結果到全域狀態
            processedReferences = data.references;

            // 渲染結果
            displayResults(data.references);
        } else {
            alert('處理失敗：' + data.error);
        }
    } catch (error) {
        console.error('錯誤:', error);
        alert('處理失敗：' + error.message);
    } finally {
        // 隱藏載入中
        loading.classList.add('hidden');
        parseBtn.disabled = false;
    }
});
```

**技術要點：**
- `async/await`：現代非同步處理
- `try/catch/finally`：完整錯誤處理
- 使用者體驗：載入中狀態、按鈕禁用

---

#### 5.2.4 結果渲染

```javascript
function displayResults(references) {
    """渲染結果到頁面"""
    // 顯示結果容器
    resultsContainer.classList.remove('hidden');

    // 更新文獻數量
    document.getElementById('result-count').textContent = references.length;

    // 清空現有結果
    const tbody = document.getElementById('results-table');
    tbody.innerHTML = '';

    // 渲染每條文獻
    references.forEach((ref, index) => {
        const row = document.createElement('tr');
        row.id = `ref-${index}`;

        // 欄位 1：編號
        const numCell = document.createElement('td');
        numCell.className = 'border px-4 py-2';
        numCell.textContent = index + 1;

        // 欄位 2：狀態標籤
        const statusCell = document.createElement('td');
        statusCell.className = 'border px-4 py-2';
        statusCell.innerHTML = getStatusBadge(ref.status, ref.confidence);

        // 欄位 3：原始輸入
        const originalCell = document.createElement('td');
        originalCell.className = 'border px-4 py-2 text-sm text-gray-600';
        originalCell.textContent = ref.original;

        // 欄位 4：格式化結果
        const formattedCell = document.createElement('td');
        formattedCell.className = 'border px-4 py-2 formatted-cell';
        formattedCell.textContent = ref.formatted_all[currentFormat];

        // 組合行
        row.appendChild(numCell);
        row.appendChild(statusCell);
        row.appendChild(originalCell);
        row.appendChild(formattedCell);

        tbody.appendChild(row);
    });
}

function getStatusBadge(status, confidence) {
    """生成狀態標籤"""
    if (status === 'enriched') {
        return '<span class="badge badge-success">已補完</span>';
    } else if (confidence > 0.7) {
        return '<span class="badge badge-info">高信心</span>';
    } else if (confidence > 0.4) {
        return '<span class="badge badge-warning">中信心</span>';
    } else {
        return '<span class="badge badge-danger">低信心</span>';
    }
}
```

**用戶體驗設計：**
- 顏色編碼：直觀顯示處理狀態
- 信心度指示：幫助用戶識別需檢查的文獻
- 表格格式：易於閱讀和比較

---

### 5.3 CSS 樣式補充

**檔案：** `static/css/style.css`

```css
/* 自定義樣式（補充 Tailwind）*/

/* 按鈕樣式 */
.btn-primary {
    @apply bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-6 rounded-lg transition duration-200;
}

.btn-secondary {
    @apply bg-gray-300 hover:bg-gray-400 text-gray-800 font-semibold py-3 px-6 rounded-lg transition duration-200;
}

/* 格式按鈕 */
.format-btn {
    @apply font-semibold py-3 px-4 rounded-lg transition duration-200;
}

.format-btn.active {
    @apply bg-blue-600 text-white;
}

.format-btn:not(.active) {
    @apply bg-gray-200 text-gray-800 hover:bg-gray-300;
}

/* 載入動畫 */
.spinner {
    display: inline-block;
    width: 48px;
    height: 48px;
    border: 4px solid rgba(59, 130, 246, 0.3);
    border-top-color: #3b82f6;
    border-radius: 50%;
    animation: spin 0.8s linear infinite;
}

@keyframes spin {
    to { transform: rotate(360deg); }
}

/* 狀態標籤 */
.badge {
    @apply inline-block px-3 py-1 text-xs font-semibold rounded-full;
}

.badge-success {
    @apply bg-green-100 text-green-800;
}

.badge-info {
    @apply bg-blue-100 text-blue-800;
}

.badge-warning {
    @apply bg-yellow-100 text-yellow-800;
}

.badge-danger {
    @apply bg-red-100 text-red-800;
}
```

---

## 第六章：部署與 DevOps

### 6.1 Docker 容器化

#### 6.1.1 Dockerfile 解析

**檔案：** `Dockerfile`

```dockerfile
# 基礎映像：Python 3.11 精簡版
FROM python:3.11-slim

# 設定工作目錄
WORKDIR /app

# 安裝系統依賴（編譯 Python 套件需要）
RUN apt-get update && apt-get install -y \
    gcc \                          # C 編譯器（某些套件需要）
    && rm -rf /var/lib/apt/lists/* # 清理快取（減小映像大小）

# 複製依賴文件
COPY requirements.txt .

# 安裝 Python 依賴
RUN pip install --no-cache-dir -r requirements.txt gunicorn

# 複製應用代碼
COPY . .

# 創建非 root 用戶（安全性最佳實踐）
RUN useradd -m -u 1000 appuser && chown -R appuser:appuser /app
USER appuser

# 暴露端口
EXPOSE 8080

# 啟動命令
CMD ["gunicorn", "--config", "gunicorn.conf.py", "wsgi:app"]
```

**最佳實踐：**
1. **精簡映像**：使用 `slim` 版本，減小大小
2. **層次優化**：先複製 `requirements.txt`，利用 Docker 快取
3. **安全性**：非 root 用戶運行
4. **清理快取**：`rm -rf /var/lib/apt/lists/*`

---

#### 6.1.2 .dockerignore 檔案

**檔案：** `.dockerignore`

```
# Python
__pycache__/
*.pyc
*.pyo
*.pyd
.Python

# 虛擬環境
venv/
env/
ENV/

# 開發文件
.git/
.gitignore
*.md
.env

# IDE
.vscode/
.idea/
*.swp

# 測試
.pytest_cache/
htmlcov/
```

**用途：** 減小 Docker 上下文大小，加快構建速度。

---

### 6.2 Gunicorn 生產配置

**檔案：** `gunicorn.conf.py`

```python
import multiprocessing

# Worker 進程數（公式：CPU * 2 + 1）
workers = multiprocessing.cpu_count() * 2 + 1

# Worker 類型
worker_class = "sync"  # 同步 worker，適合 I/O 密集型

# 綁定地址
bind = "0.0.0.0:8080"

# 超時設定
timeout = 120      # 120 秒超時（API 請求可能較慢）
keepalive = 5      # Keep-Alive 連線時間

# 日誌
accesslog = "-"    # 存取日誌（- 表示 stdout）
errorlog = "-"     # 錯誤日誌
loglevel = "info"  # 日誌級別

# 優雅重啟
graceful_timeout = 30

# 預載應用（節省記憶體）
preload_app = True
```

**配置說明：**
- **Worker 數**：平衡併發與資源
- **Timeout**：考慮 CrossRef API 延遲
- **日誌**：導向 stdout，便於 Render 收集

---

### 6.3 Render 部署

#### 6.3.1 Render 配置檔

**檔案：** `render.yaml`

```yaml
services:
  - type: web
    name: reference-formatter
    env: python
    region: singapore  # 選擇離用戶近的區域
    plan: free         # 免費方案

    # 構建命令
    buildCommand: pip install -r requirements.txt

    # 啟動命令
    startCommand: gunicorn --config gunicorn.conf.py wsgi:app

    # 環境變數
    envVars:
      - key: PYTHON_VERSION
        value: 3.11.0

      - key: FLASK_ENV
        value: production

      - key: SECRET_KEY
        generateValue: true  # Render 自動生成安全密鑰

      - key: CROSSREF_EMAIL
        value: your-email@example.com  # 在儀表板修改

    # 健康檢查
    healthCheckPath: /health
```

---

#### 6.3.2 部署流程

**步驟 1：準備 Git 倉庫**
```bash
git init
git add .
git commit -m "Initial commit"
git remote add origin https://github.com/username/reference-formatter.git
git push -u origin main
```

**步驟 2：在 Render 創建服務**
1. 登入 Render Dashboard
2. 點擊「New +」→「Web Service」
3. 連接 GitHub 倉庫
4. Render 自動檢測 `render.yaml`
5. 點擊「Create Web Service」

**步驟 3：配置環境變數**
- `SECRET_KEY`：自動生成（已配置）
- `CROSSREF_EMAIL`：在儀表板修改為你的 email

**步驟 4：部署**
- Render 自動觸發部署
- 監控日誌：Dashboard → Logs
- 訪問應用：`https://reference-formatter.onrender.com`

---

#### 6.3.3 自動化部署

**Git Hook 設置：**
```bash
# 每次 push 到 main 分支，Render 自動部署
git push origin main
```

**部署日誌範例：**
```
==> Building...
    Installing dependencies from requirements.txt
    ✓ Flask==3.0.0
    ✓ python-docx==1.1.0
    ✓ gunicorn==21.2.0
==> Starting service...
    [2024-12-31 16:00:00] [INFO] Starting gunicorn 21.2.0
    [2024-12-31 16:00:00] [INFO] Listening at: http://0.0.0.0:8080
    [2024-12-31 16:00:00] [INFO] Using worker: sync
    [2024-12-31 16:00:00] [INFO] Booting worker with pid: 123
==> Service is live!
```

---

### 6.4 環境變數管理

#### 6.4.1 開發環境（.env 檔案）

**檔案：** `.env.example`（模板，實際 `.env` 不提交到 Git）

```bash
# Flask 環境
FLASK_ENV=development

# 安全密鑰（開發環境可用弱密鑰）
SECRET_KEY=dev-secret-key

# CrossRef API
CROSSREF_EMAIL=your-email@example.com

# 日誌級別
LOG_LEVEL=DEBUG
```

**使用：**
```python
from dotenv import load_dotenv
load_dotenv()  # 載入 .env

SECRET_KEY = os.environ.get('SECRET_KEY')
```

---

#### 6.4.2 生產環境（Render）

**在 Render Dashboard 設置：**
- Environment → Add Environment Variable
- Key: `SECRET_KEY`, Value: `[自動生成的長字串]`
- Key: `FLASK_ENV`, Value: `production`

---

## 第七章：問題偵錯案例集

### 7.1 SECRET_KEY 驗證問題

#### 問題描述
生產環境啟動失敗，錯誤訊息：
```
ValueError: 生產環境必須設定有效的 SECRET_KEY 環境變量！
```

#### 原因分析
**錯誤代碼（config.py）：**
```python
class ProductionConfig(Config):
    DEBUG = False

    # ❌ 錯誤：在類定義時檢查
    if not os.environ.get('SECRET_KEY'):
        raise ValueError("SECRET_KEY 未設定！")
```

**問題：**
- 類定義在模組匯入時執行
- 此時 Render 的環境變數可能尚未載入
- 導致檢查失敗

#### 解決方案
**正確代碼（app.py）：**
```python
# 載入配置
env = os.environ.get('FLASK_ENV', 'development')
app.config.from_object(config[env])

# ✅ 正確：在運行時檢查
if env == 'production':
    if not app.config.get('SECRET_KEY') or \
       app.config['SECRET_KEY'] == 'dev-secret-key-please-change-in-production':
        raise ValueError("生產環境必須設定有效的 SECRET_KEY！")
```

#### 學習點
- 理解 Python 類定義時機（import time vs runtime）
- 環境變數載入順序
- 生產環境安全檢查的正確時機

---

### 7.2 CORS 跨域問題

#### 問題描述
前端 AJAX 請求失敗：
```
Access to fetch at 'http://localhost:8080/parse' from origin 'http://localhost:3000'
has been blocked by CORS policy
```

#### 原因分析
- 前後端分離開發（不同端口）
- 瀏覽器同源政策限制

#### 解決方案
**方案 1：Flask-CORS（開發環境）**
```python
from flask_cors import CORS

app = Flask(__name__)
CORS(app)  # 允許所有來源（僅開發環境）
```

**方案 2：配置特定來源（生產環境）**
```python
CORS(app, resources={
    r"/api/*": {
        "origins": ["https://yourdomain.com"],
        "methods": ["GET", "POST"],
        "allow_headers": ["Content-Type"]
    }
})
```

**方案 3：同源部署（最佳）**
- 前後端部署在同一域名下
- Render 自動處理 HTTPS

---

### 7.3 API 速率限制

#### 問題描述
CrossRef API 返回 429 錯誤：
```
HTTP 429: Too Many Requests
```

#### 原因分析
- CrossRef 限制每秒請求數
- 批次處理時觸發限制

#### 解決方案
**已實現的指數退避：**
```python
elif response.status_code == 429:
    wait_time = 2 ** attempt  # 1s, 2s, 4s, 8s...
    logger.warning(f"速率限制，等待 {wait_time} 秒後重試...")
    time.sleep(wait_time)
```

**優化建議：**
1. **快取機制**：相同 DOI 不重複查詢
   ```python
   from functools import lru_cache

   @lru_cache(maxsize=1000)
   def query_by_doi_cached(doi: str):
       return self.query_by_doi(doi)
   ```

2. **批次限流**：控制並發請求
   ```python
   import time

   for ref in references:
       enrich_reference(ref)
       time.sleep(0.1)  # 每次請求間隔 100ms
   ```

---

### 7.4 記憶體溢出（大文件匯出）

#### 問題描述
匯出 1000+ 條文獻時，Render 服務崩潰。

#### 原因分析
- 免費方案記憶體限制：512MB
- 大量文獻一次性載入記憶體

#### 解決方案
**分批處理：**
```python
@app.route('/export/docx', methods=['POST'])
def export_docx():
    data = request.get_json()
    references = data.get('references', [])

    # 限制單次匯出數量
    if len(references) > 500:
        return jsonify({
            'error': '單次最多匯出 500 條文獻，請分批處理'
        }), 400

    # 流式寫入
    doc = Document()
    for ref in references:
        doc.add_paragraph(ref['formatted'])
        # 不累積在記憶體

    buffer = io.BytesIO()
    doc.save(buffer)
    buffer.seek(0)

    return send_file(buffer, ...)
```

---

### 7.5 Render 冷啟動問題

#### 問題描述
免費方案閒置 15 分鐘後自動休眠，首次訪問慢（10-30 秒）。

#### 解決方案
**方案 1：定時 Ping（外部）**
```bash
# 使用 cron-job.org 每 10 分鐘請求一次
curl https://your-app.onrender.com/health
```

**方案 2：接受冷啟動（推薦）**
- 免費方案的合理限制
- 首頁添加提示：「首次載入可能需要 10-20 秒」

**方案 3：升級付費方案**
- Starter 方案：$7/月，無冷啟動

---

## 第八章：性能優化與最佳實踐

### 8.1 前端優化

#### 8.1.1 資源壓縮
```html
<!-- 生產環境使用壓縮版 -->
<script src="https://cdn.tailwindcss.com?plugins=forms,typography"></script>

<!-- 或自建 Tailwind（按需生成，體積更小）-->
<link href="/static/css/tailwind.min.css" rel="stylesheet">
```

#### 8.1.2 懶載入
```javascript
// 結果表格虛擬滾動（處理大量文獻）
function renderVisibleRows(startIndex, endIndex) {
    const tbody = document.getElementById('results-table');
    tbody.innerHTML = '';

    for (let i = startIndex; i < endIndex; i++) {
        if (processedReferences[i]) {
            tbody.appendChild(createRow(processedReferences[i]));
        }
    }
}
```

---

### 8.2 後端優化

#### 8.2.1 非同步 API 查詢
```python
import asyncio
import aiohttp

async def enrich_references_async(references):
    """非同步批次補完"""
    async with aiohttp.ClientSession() as session:
        tasks = [
            enrich_single(session, ref)
            for ref in references
        ]
        return await asyncio.gather(*tasks)
```

#### 8.2.2 Redis 快取
```python
import redis
import json

redis_client = redis.Redis(host='localhost', port=6379, db=0)

def query_by_doi_cached(doi):
    # 檢查快取
    cached = redis_client.get(f"doi:{doi}")
    if cached:
        return json.loads(cached)

    # 查詢 API
    result = query_by_doi(doi)

    # 存入快取（1 天有效期）
    redis_client.setex(
        f"doi:{doi}",
        86400,  # 1 天
        json.dumps(result)
    )

    return result
```

---

### 8.3 安全性最佳實踐

#### 8.3.1 輸入驗證
```python
@app.route('/parse', methods=['POST'])
def parse_references():
    data = request.get_json()

    # 驗證輸入長度（防 DoS）
    text = data.get('text', '')
    if len(text) > 50000:  # 限制 50KB
        return jsonify({'error': '輸入過長'}), 400

    # 驗證格式
    format_style = data.get('format', 'apa')
    if format_style not in ['apa', 'mla', 'chicago', 'harvard']:
        return jsonify({'error': '無效的格式'}), 400
```

#### 8.3.2 速率限制
```python
from flask_limiter import Limiter

limiter = Limiter(
    app,
    key_func=lambda: request.remote_addr,
    default_limits=["100 per hour"]
)

@app.route('/parse', methods=['POST'])
@limiter.limit("10 per minute")  # 每分鐘 10 次
def parse_references():
    ...
```

---

## 第九章：延伸學習路線圖

### 9.1 功能擴展

#### 9.1.1 支援更多格式
- [ ] IEEE
- [ ] Vancouver
- [ ] AMA (American Medical Association)
- [ ] 自定義格式（JSON 配置）

#### 9.1.2 PDF 處理
```python
# 使用 PyPDF2 提取參考文獻
import PyPDF2

def extract_references_from_pdf(pdf_file):
    reader = PyPDF2.PdfReader(pdf_file)
    text = ""
    for page in reader.pages:
        text += page.extract_text()

    # 定位參考文獻區塊
    ref_section = extract_reference_section(text)
    return parse_multiple(ref_section)
```

---

### 9.2 技術升級

#### 9.2.1 前端框架化（React）
```jsx
// components/ReferenceFormatter.jsx
import React, { useState } from 'react';

function ReferenceFormatter() {
    const [format, setFormat] = useState('apa');
    const [references, setReferences] = useState([]);

    const parseReferences = async (text) => {
        const response = await fetch('/parse', {
            method: 'POST',
            body: JSON.stringify({ text, format }),
        });
        const data = await response.json();
        setReferences(data.references);
    };

    return (
        <div>
            <FormatSelector format={format} onChange={setFormat} />
            <InputArea onParse={parseReferences} />
            <ResultsTable references={references} format={format} />
        </div>
    );
}
```

#### 9.2.2 資料庫整合（PostgreSQL）
```python
# models.py
from flask_sqlalchemy import SQLAlchemy

db = SQLAlchemy()

class Reference(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'))
    doi = db.Column(db.String(255), unique=True)
    title = db.Column(db.Text)
    authors = db.Column(db.JSON)
    year = db.Column(db.Integer)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)

class User(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    email = db.Column(db.String(255), unique=True)
    references = db.relationship('Reference', backref='user')
```

---

### 9.3 測試與 CI/CD

#### 9.3.1 單元測試（pytest）
```python
# tests/test_parser.py
import pytest
from modules.parser import ReferenceParser

def test_doi_extraction():
    parser = ReferenceParser()
    text = "Article with DOI: 10.1038/nature12373"
    result = parser.parse_reference(text)
    assert result['doi'] == '10.1038/nature12373'

def test_year_extraction():
    parser = ReferenceParser()
    text = "Smith, J. (2020). Article title."
    result = parser.parse_reference(text)
    assert result['year'] == '2020'
```

#### 9.3.2 GitHub Actions CI
```yaml
# .github/workflows/test.yml
name: Test

on: [push, pull_request]

jobs:
  test:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v2
      - uses: actions/setup-python@v2
        with:
          python-version: '3.11'
      - run: pip install -r requirements.txt
      - run: pytest tests/
```

---

## 總結：從專案學到的關鍵技能

### 技術技能
✅ **全端開發**：Flask + Vanilla JS + Tailwind
✅ **API 整合**：RESTful API、第三方 API、錯誤處理
✅ **設計模式**：策略模式、工廠模式、模板方法
✅ **容器化**：Docker、環境分離、部署自動化
✅ **正則表達式**：文本解析、模式匹配

### 軟技能
✅ **需求分析**：從問題到解決方案
✅ **架構設計**：模組化、關注點分離
✅ **除錯思維**：系統性排查、日誌分析
✅ **文檔撰寫**：清晰的 README、部署指南

### 工程實踐
✅ **版本控制**：Git 工作流程
✅ **配置管理**：環境變數、敏感資訊保護
✅ **性能優化**：快取、非同步、資源壓縮
✅ **安全性**：輸入驗證、速率限制、HTTPS

---

## 附錄：常用命令速查表

### 開發環境
```bash
# 創建虛擬環境
python3 -m venv venv
source venv/bin/activate  # Windows: venv\Scripts\activate

# 安裝依賴
pip install -r requirements.txt

# 運行開發伺服器
python app.py

# 凍結依賴
pip freeze > requirements.txt
```

### Docker
```bash
# 構建映像
docker build -t reference-formatter .

# 運行容器
docker run -p 8080:8080 reference-formatter

# 查看日誌
docker logs <container-id>
```

### Git
```bash
# 初始化倉庫
git init
git add .
git commit -m "Initial commit"

# 推送到 GitHub
git remote add origin https://github.com/username/repo.git
git push -u origin main

# 查看狀態
git status
git log --oneline --graph
```

---

**祝你學習順利！有任何問題歡迎隨時詢問。**
